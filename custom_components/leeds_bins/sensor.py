...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    if config is not None:
        async_add_entities([leedsbins_Sensor(hass, config)], True)
...
async def async_setup_entry(hass, config, async_add_devices):
    ...
    if len(config.data) > 0:
        async_add_devices([leedsbins_Sensor(hass, config.data)], True)


class leedsbins_Sensor(Entity):
    ...
    def __init__(self, hass, config):
        ...
        self.entity_id = async_generate_entity_id(
            ENTITY_ID_FORMAT, "ics_" + str(config.get(CONF_ID)), hass=hass
        )
    ...
    def check_fix_date_tz(self, event):
        ...
        if isinstance(event.dt, datetime.date) and not (
            isinstance(event.dt, datetime.datetime)
        ):
                ...
                if "UNTIL" in event["RRULE"]:
                    ...
                    if type(event["DTSTART"].dt) != type(event["RRULE"]["UNTIL"][0]):
                        if type(event["DTSTART"].dt) == datetime.datetime:
                            ...
                            event["RRULE"]["UNTIL"][0] = datetime.datetime(
                                d.year, d.month, d.day
                            )
                        else:
                            event["RRULE"]["UNTIL"][0] = event["RRULE"]["UNTIL"][
                                0
                            ].date()
                        fix += 1
                    # tz fixing
                    if type(event["RRULE"]["UNTIL"][0]) == datetime.datetime:
                        event["RRULE"]["UNTIL"][0] = event["RRULE"]["UNTIL"][0].replace(
                            tzinfo=datetime.UTC
                        )
        return fix
    ...
    async def get_data(self):
        ...
        try:
            ...
            try:
                reoccuring_events = sorted(
                    reoccuring_events, key=lambda x: x["DTSTART"].dt, reverse=False
                )
            except Exception:
                self.exc()

            self.ics["pickup_date"] = "no next event"
            self.ics["extra"]["last_updated"] = datetime.datetime.now(
                get_localzone()
            ...
            if len(reoccuring_events) > 0:
                for e in reoccuring_events:
                    ...
                    if event_summary:
                        if (
                            event_summary.lower().startswith(
                                self.fix_text(self._sw).lower()
                            )
                            and event_summary.lower().find(
                                self.fix_text(self._contains).lower()
                            )
                            >= 0
                            and self.matches_regex(event_summary)
                        ):
                            if (event_date > now) or (
                                self._show_ongoing and event_end_date > now
                            ):
                                ...
                                if n_skip > 0 or skip_et is not None:
                                    ...
                                    if (
                                        skip_et is not None
                                        and skip_et == event_date
                                        and self._group_events
                                    ):
                                ...
                                if et is None:
                                    self.ics["pickup_date"] = event_date.strftime(
                                        self._timeformat
                                    )
                                    self.ics["extra"]["remaining"] = (
                                        event_date.date() - now.date()
                                    ).days
                                    self.ics["extra"]["description"] = event_summary
                                    self.ics["extra"]["start"] = event_date.astimezone()
                                    self.ics["extra"]["end"] = (
                                        event_end_date.astimezone()
                                    )
                                    if "LOCATION" in e:
                                        self.ics["extra"]["location"] = self.fix_text(
                                            e["LOCATION"]
                                        )
                                    et = event_date
            ...
            self.ics["extra"]["reload_at"] = now.replace(
            ...
            self.ics["extra"]["reload_at"] = self.ics["extra"]["reload_at"].replace(
                microsecond=0
            )
    ...
    async def async_update(self):
        ...
        try:
            # first run
            if self.ics["extra"]["reload_at"] is None or self.ics["extra"][
                "reload_at"
